...
const BackendUrlInput = ({
    ...
    const [backendUrl, setBackendUrl] = useState('https://standards-overseas-testimonials-emperor.trycloudflare.com');
    const showInput = qs.parse(window.location.search, {ignoreQueryPrefix: true}).custom;
    useEffect(() => {
    ...
    useEffect(() => {
        if (!backendUrl){
                onChange('https://standards-overseas-testimonials-emperor.trycloudflare.com');
        } else {
                onChange(backendUrl);
        }
    }, []);
        ...
        <Grid container spacing={1} alignContent="center">
            <Grid item xs={10}>
                <TextField className={classes.inputBackend} fullWidth id="standard-basic"
                           ...
                           helperText={!isValidBackendEndpoint && backendUrl !== '' && "No running Text2Image server with this URL (did you include the http prefix?)"}
