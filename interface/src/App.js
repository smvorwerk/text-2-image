...
import { callText2ImageService } from "./backend_api";
...
const App = ({ classes }) => {
    ...
    function enterPressedCallback(promptText) {
        console.log('API call to Text2Image web service with the following prompt [' + promptText + ']');
        setApiError('')
        setIsFetchingImgs(true)
        callText2ImageService(backendUrl, promptText, imagesPerQuery).then((response) => {
            ...
            if (notificationsOn) {
                new Notification(
                    "Your Text2Image images are ready!",
                    {
            ...
            console.log('Error querying Text2Image service.', error)
            if (error.message === 'Timeout') {
                setApiError('Timeout querying Text2Image service (>1min). Consider reducing the images per query or use a stronger backend.')
            } else {
                setApiError('Error querying Text2Image service. Check your backend server logs.')
            }
            setIsFetchingImgs(false)
        })
    }
        ...
        <div className={classes.root}>
            <div className={classes.title}>
                <Typography variant="h3">
                    Text2Image Playground <span role="img" aria-label="sparks-emoji">✨</span>
                </Typography>
            </div>

            {!validBackendUrl && <div>
                <Typography variant="body1" color="textSecondary">
                    Put your Text2Image backend URL to start
                </Typography>
            </div>}
